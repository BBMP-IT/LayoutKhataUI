    ...
    const fetchSiteDetails = async (LKRS_ID) => {
        try {
            ...
            if (response.length >= 0) {
                ...
                setAllSites(releasedSites); // Only released sites shown in table

                // Show/hide release button based on unreleased sites existing
                // if (response.every(site => site.sitE_IS_SITE_RELEASED === true)) {
                //     setShouldShowReleaseButton(false); // All released → hide button
                // } else {
                //     setShouldShowReleaseButton(true); // Some not released → show button
                // }

                console.table("Released sites only", releasedSites);
            } else {
                Swal.fire({
                    ...
                    <div style={{ border: "2px solid black", padding: 20, background: "#fff", width: "90%", maxWidth: 800, boxShadow: "0 4px 10px rgba(0,0,0,0.1)" }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", borderBottom: "2px solid black", paddingBottom: 10, marginBottom: 20 }}>
                            ...
                            <div>
                                ...
                                <h4 style={{ margin: 0, fontSize: 20, textAlign: "center", fontWeight: "bold" }}>Acknowledgement</h4>
                            </div>
                        </div>
                        ...
                        <div className="status14">
                            <p>Your application for New Khata for the property in the schedule below is received successfully and Non-Transactable Provisional New Khata with temporary ePID has been generated, which you can download after upto-date payment of property tax.</p>
                        </div>
                        <div className="statusNot14">
                            <p>Non-Transactable Provisional New Khata at https://BBMPeAasthi.Karnataka.gov.in (Using your temporary ePID)</p>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '20px' }} border="1">
                                    ...
                                    <tbody>
                                        {allSites.map((site, index) => {
                                            // Format dimension as "50ft + 40ft"
                                            const dimensions = site.siteDimensions
                                                .map((dim) => `${dim.sitediM_SIDEINFT}ft`)
                                                .join(' + ');
                                            ...
                                            return (
                                                <tr key={site.sitE_ID}>
                                                    ...
                                                    <td style={{ padding: '8px', border: '1px solid #000' }}>{site.sitE_NO_OF_SIDES}</td>
                                                    <td style={{ padding: '8px', border: '1px solid #000' }}>
                                                        {(() => {
                                                            ...
                                                            return `${totalFt} [Sq.ft], ${totalMt.toFixed(2)} [Sq.mtr]`;
                                                        })()}
                                                    </td>
                                                    <td style={{ padding: '8px', border: '1px solid #000' }}>{site.sitE_AREAINSQFT} sqft</td>
                                                    <td style={{ padding: '8px', border: '1px solid #000' }}>
                                                        {site.sitE_IS_SITE_RELEASED ? 'Yes' : 'No'}
                                                    </td>
                                                </tr>
                                            );
                                        })}
